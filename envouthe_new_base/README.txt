...
et pour les champs "action_open" et "action_click", qui sont les données chaudes, je les met à côté pour l'instant, ils ne vont pas passer comme input dans la machine learning.
...
Machine Learning
  J’ai d’abord défini le churn comme le label, et j'ai utiliser ExtraTreesClassifier et SelectFromModel à choisir les features les plus importants.
  Et j'ai regoupé les champs "action_open" et "action_click" à la fin de dataset, donc, le structure de dataset devient features+label+données_chaudes.
  ...
  Et enfin, j’ai sauvegardé les prédictions dans les fichiers différents. Les fichiers comprennent l’id d’abonnement, le label réel, le label prédit, le label prédit avec probabilité, les features utilisés comme input, et les données chaudes.
